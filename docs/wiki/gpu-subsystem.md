...
- [rfc04-bump-bake.md](rfc04-bump-bake.md) — historical design notes for
...
JavaScript wall-clock time. Local probes showed GPU-derived FPS limits
matching independent RAF cadence on representative views, so GPU timing
is the preferred bottleneck metric when enabled. With GPU timing
disabled, `renderMs` and `limitFps` are CPU-frame metrics. With GPU
timing enabled and valid samples available, `renderMs` and `limitFps`
report the CPU/GPU bottleneck while `cpuMs` and `gpuMs` expose the two
components.
