...
<a id="backlog-66"></a>
## 66. PERF: adapt the texel fit to eviction pressure

**Opened:** 2026-09-09
**Status:** open
**Related:** `src/map/draw.js`, `src/map/map.ts` (`cacheBudgets`),
`src/map/geodata-heightcoding-job.ts`,
[backlog 62](backlog-archive.md#backlog-62)

With the cache budgets sized from the canvas, a phone runs at the
floors and a tablet at about half the FullHD baseline. On a coarse,
label-heavy view the GPU cache is then smaller than the view's working
set: entries are evicted before they are drawn again, re-uploaded, and
in store mode republished from the retained job. The publication cap
bounds the queue, but every eviction costs a re-upload and, in store
mode, a republication, so a coarse pan stays slow while the cache is
smaller than the view.

The lever is `texelSizeFit`: coarser tiles mean fewer of them, and a
view that fits its cache stops churning. Detect the storm from the
cache itself, entries evicted within seconds of insertion or before
their first draw, raise the fit while it lasts, and let it decay once
evictions settle. Label density already follows the physical screen
size; the texel fit following the memory the screen can afford is the
same idea. Measure on a device that runs at the floors, over a coarse,
label-heavy pan.
