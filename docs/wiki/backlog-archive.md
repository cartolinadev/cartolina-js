...
**Opened:** 2026-04-14
**Status:** fixed 2026-08-24 — see Resolution below.
...
Fixed by removing the `'float'` mode, not by feeding it a better anchor
height as "Suggested fix direction" imagined. `checkVisibility(pos)` now
does the depth comparison for an explicit height only — the path that
already worked. The caller resolves the anchor height through
`queryTerrainElevation`, backed by the RFC 13 elevation store, and
